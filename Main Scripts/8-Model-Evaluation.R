...
pacman::p_load(tidyverse,tidymodels,workflowsets,tidyposterior,
               probably,doFuture,furrr,themis,
...
best_mods = 
  ...
  group_by(model) |>
  mutate(roc_rank   = dense_rank(1-best_roc_auc)) |>
  mutate(brier_rank = dense_rank(best_brier)) |>
  mutate(combo_rank = dense_rank(roc_rank + brier_rank)) |>
  slice_max(best_roc_auc) 
  # slice_min(combo_rank)

#The best AUROC happens to co-occur with the best combo of brier score and AUROC, which is handy
...
tab_mods = 
  ...
  #We can work out the Brier score as requested by a reviewer
...
#However, we ought to bootstrap to get confidence intervals for these measures to get
#a sense of how variable they are
...
bootstrap_wf_metrics <- function(wf,data_set,n_boot = 999){
...
#Now we asses the calibration of our selected best model
...
#Note here the threshold used for classification is 0.5
...
tab_age = 
  ...
  mutate(roc   = map_dbl(data,~.x |> 
                           roc_auc(group,`.pred_ND-GC`,event_level = "second") |> 
                           pull(.estimate) |> 
                           round(digits = 3))) |>
  select(-c(data)) 
...
p_roc = 
  ...
  unnest(roc_curve_d) |>
  mutate(model = factor(model,levels = c("nnet","en","rf","svm"))) |>
...
#Use model based VI for the EN models
best_mods |> 
  select(model,test_fit) |> 
  unnest(test_fit) |>
  filter(model == "rf") |>
...
#Perform permutation testing on the improtance of variables in the final model
...
best_model_varset = 
  best_model_importance$best_wf_final[[1]] |>
...
p_best_var_imp = 
  ...
  left_join(d_var |> select(variable,short_name),by = "variable") |>
  arrange(-dropout_loss) |>
...
#,title = "Best RF Model",subtitle = "Vertical Line = Full Model"

p_best_var_imp


#Out of interest, how many variables have a dropout loss greater than the mean - all of them
#nice
#
# best_model_parts |>
#   as_tibble() |> 
...
ggsave("C://Users/nadon/OneDrive - University of Bristol/Documents/CNV Item Reduction/Figures/Figure Parts/figure_2.pdf",
       p_perf,width = 5, height = 3)

ggsave("C://Users/nadon/OneDrive - University of Bristol/Documents/CNV Item Reduction/Figures/Figure Parts/figure_2_vi.pdf",
       p_best_var_imp + plot_annotation(tag_levels = "A"),width = 5, height = 2)
...
#As an aside, how good is the performance of a logistic regression
#model fit with the RF variables at classification?
...
#Plot probs
cnv_results  |> ggplot(aes(`.pred_ND-GC`,fill = .pred_class)) + geom_histogram(binwidth = .05)
